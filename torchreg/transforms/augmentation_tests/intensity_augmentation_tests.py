...
if __name__ == '__main__':
    ...
    intensity_augment = transforms.RandomIntenityShift(instensity_change_std=60, size_std=800, filter_count=15, downsize=8)
    intensity_augment.randomize()
    batch_intensity = intensity_augment(batch)

    # save augmented images
    save_png('./torchreg/transforms/augmentation_tests/post-transform/noise.png', batch_noise)
    save_png('./torchreg/transforms/augmentation_tests/post-transform/intensity.png', batch_intensity)
    ...
    intensity_augment = transforms.RandomIntenityShift(instensity_change_std=60, size_std=200, filter_count=20, downsize=8)
    intensity_augment.randomize()
    batch_intensity = intensity_augment(batch)

    # save augmented images
    save_nii('./torchreg/transforms/augmentation_tests/post-transform/noise.nii.gz', nii, batch_noise)
    save_nii('./torchreg/transforms/augmentation_tests/post-transform/intensity.nii.gz', nii, batch_intensity)
