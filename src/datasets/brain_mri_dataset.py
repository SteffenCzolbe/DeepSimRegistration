...
def BrainMRIDataset(path: str, split: str, pairs=False):
    ...
    image_nii_files = list(map(lambda s: os.path.join(path, 'data', s, 'brain_aligned.nii.gz'), subjects))
    image_nii_label_files = list(map(lambda s: os.path.join(path, 'data', s, 'seg_coalesced_aligned.nii.gz'), subjects))
    ...
    if pairs:
        return NiiAtlasDataset(atlas_nii_file, atlas_nii_label_file, image_nii_files, image_nii_label_files, min_intensity, max_intensity)
    else:
        return NiiDataset(image_nii_files, image_nii_label_files, min_intensity, max_intensity)
