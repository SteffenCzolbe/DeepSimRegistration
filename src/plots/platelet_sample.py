...
def plot_platelet(fig, row, col, I, S, inv_flow=None, title=None):
    ...
    if inv_flow is not None:
        fig.plot_transform_grid(row, col, inv_flow[0], interval=10, linewidth=0.5, color='#000000FF' , overlay=True)
...
for j, loss_function in enumerate(LOSS_FUNTION_ORDER):
    ...
    # plot aligned image
    plotfun(fig, 0, j, I_m, S_m, inv_flow=inv_flow, title=LOSS_FUNTION_CONFIG[loss_function]['display_name'])
