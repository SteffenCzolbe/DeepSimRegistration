...
for dataset in results.keys():
    for loss_function in results[dataset].keys():
        ...
        _, p_value = scipy.stats.wilcoxon(results[dataset]['deepsim']['dice_overlap'], results[dataset][loss_function]['dice_overlap'], alternative='greater')

        # accept when statistically significant p < 0.05
        results[dataset][loss_function]['statistically_significantly_worse_than_deepsim_pval'] = p_value
        results[dataset][loss_function]['statistically_significantly_worse_than_deepsim'] = p_value < 0.05
