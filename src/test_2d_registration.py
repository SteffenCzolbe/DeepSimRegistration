...
def main(hparams):
    ...
    for i in tqdm(range(len(test_set)), desc="creating tif image"):
        ...
        fig.plot_transform_vec(
            0, 0, -flow[0], interval=10, arrow_length=1.0, linewidth=2.0, overlay=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default="./weights/phc-u373/registration/ncc+supervised/weights.ckpt",
        help="model checkpoint to initialize with",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="./out/phc-u373/registration/ncc+supervised.tif",
