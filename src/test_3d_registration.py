...
def main(hparams):
    ...
    for i in tqdm(range(1), desc="registering.."):
        ...
        f.save_tensor_as_nii(
            os.path.join(hparams.out, f"{i}i_0.nii.gz"),
            I_0[0],
            affine=affine,
            dtype=np.float32,
        )
        f.save_tensor_as_nii(
            os.path.join(hparams.out, f"{i}s_0.nii.gz"),
            S_0[0],
            affine=affine,
            dtype=np.uint8,
        )
        f.save_tensor_as_nii(
            os.path.join(hparams.out, f"{i}i_1.nii.gz"),
            I_1[0],
            affine=affine,
            dtype=np.float32,
        )
        f.save_tensor_as_nii(
            os.path.join(hparams.out, f"{i}s_1.nii.gz"),
            S_1[0],
            affine=affine,
            dtype=np.uint8,
        )
        f.save_tensor_as_nii(
            os.path.join(hparams.out, f"{i}i_m.nii.gz"),
            I_m[0],
            affine=affine,
            dtype=np.float32,
        )
        f.save_tensor_as_nii(
            os.path.join(hparams.out, f"{i}s_m.nii.gz"),
            S_m[0],
            affine=affine,
            dtype=np.uint8,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights",
        type=str,
        default="./weights/brain-mri/registration/ncc+supervised/weights.ckpt",
        help="model checkpoint to initialize with",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="./out/brain-mri/registration/ncc+supervised/",
